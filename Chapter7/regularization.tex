...
如果我们使用\gls{hessian} $\MH$为对角正定矩阵的假设（与$L^1$\gls{regularization}分析时一样），重新考虑这个等式，我们发现
...
\gls{dropout}，一个将在\secref{sec:dropout}介绍的强大\gls{regularization}策略，可以被看作是通过添加\emph{乘性}噪声构建新输入的过程。
