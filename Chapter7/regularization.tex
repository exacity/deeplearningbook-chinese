...
\gls{deep_network}的较低层（无论是监督前馈的，还是包括向下箭头的生成组件）可以跨这样的任务共享，而任务特定的参数（分别与从$\Vh^{(1)}$和$\Vh^{(2)}$进入和发出的权重）可以在共享表示$\Vh^{(\text{shared})}$之上学习。
...
在\gls{unsupervised_learning}情景中，一些顶层因素不与输出任务$(\Vh^{(3)})$的任意一个关联是有意义的：这些因素可以解释一些输入变化但与预测$\RVy^{(1)}$或$\RVy^{(2)}$不相关。
}
...
\begin{algorithmic}
\STATE 令 $n$ 为评估间隔的步数。
\STATE 令 $p$ 为 ``耐心(patience)''，即观察到较坏的验证集表现这么多次后放弃。 
...
理想情况下，这是可以并行在与主训练过程分离的机器，或独立的CPU，或独立的GPU上完成。
...
这个过程在\algref{alg:early_stopping_continue}中更正式地展示。
