...
\gls{dataset}是指很多\gls{example:chap5}组成的集合，如\secref{sec:the_task_t}的定义。有时我们也将\gls{example:chap5}称为\firstgls{data_points}。
...
这样做的一种直观的方式（我们将在后续的\secref{sec:conditional_log_likelihood_and_mean_squared_error}说明其合法性）是最小化训练集上的\gls{mean_squared_error}，$\text{MSE}_{\text{train}}$。
...
一般地，感兴趣的量可以是单个参数，或是某些参数模型中一个向量参数，例如\secref{sec:example_linear_regression}\gls{linear_regression}中的权重，但是也有可能是整个函数。
...
\gls{linear_regression}实例（\secref{sec:example_linear_regression}中讨论的）和多项式回归实例（\secref{sec:capacity_overfitting_and_underfitting}中讨论的）都既可以被解释为估计参数$\Vw$，又可以解释为估计从$\Vx$到$y$的函数映射$\hat{f}$。
...
\begin{align}
    ...
    &= \frac{m-1}{m} \sigma^2
\end{align}
回到\eqnref{eq:5.37}，我们得到结论$\hat{\sigma}^2_m$的偏差是$-\frac{\sigma^2}{m}$。
...
这是估计普遍的共同性质，在探讨一致性（参考\secref{sec:consistency}）时，我们会继续讨论。
...
在合适的条件下，\gls{maximum_likelihood_estimation}具有一致性（参考\secref{sec:consistency}），意味着训练\gls{example:chap5}数目趋向于无穷时，参数的\gls{maximum_likelihood_estimation}收敛到参数的真实值。
这些条件是：
\begin{itemize}
...
最近邻中，每个训练\gls{example:chap5}至多用于定义一个区间，如\figref{fig:chap5_empty10}所示。
...
\centerline{\includegraphics{figure.pdf}}
\else
\centerline{\includegraphics{Chapter5/figures/empty10}}
\fi
\caption{TODO}
\label{fig:chap5_empty10}
\end{figure}
...
每一维都对应着局部的变动方向。
如\figref{fig:chap5_empty11}所示，训练数据位于二维空间中的一维流形中。
...
\centerline{\includegraphics{figure.pdf}}
\else
\centerline{\includegraphics{Chapter5/figures/empty11}}
\fi
\caption{TODO}
\label{fig:chap5_empty11}
\end{figure}
...
这个一般性原则用于很多情况下。
\figref{fig:chap5_empty13}展示了包含脸的\gls{dataset}的流形结构。
...
\centerline{\includegraphics{figure.pdf}}
\else
\centerline{\includegraphics{Chapter5/figures/empty13}}
\fi
\caption{TODO}
\label{fig:chap5_empty13}
\end{figure}
