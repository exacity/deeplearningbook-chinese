...
\label{subsec:the_performance_measure_p}
...
统计学家和机器学习研究者使用很久的\gls{dataset}之一是Iris（鸢尾花卉）\gls{dataset}\citep{Fisher-1936}。
...
\label{subsec:example_linear_regression}
...
\section{容量，过拟合和欠拟合}
\label{subsec:capacity_overfitting_and_underfitting}
...
\label{subsec:the_no_free_lunch_theorem}
...
\label{subsec:trading_off_bias_and_variance_to_minimize_mean_squared_error}
偏差和方差衡量估计的两个不同误差来源。
偏差衡量离真实函数或参数的误差期望。
...
\label{subsec:conditional_log_likelihood_and_mean_squared_error}
...
\subsection{最大似然的性质}
\label{subsec:properties_of_maximum_likelihood}
...
\subsection{\gls{MAP}（\glssymbol{MAP}）估计}
\label{subsec:maximum_a_posteriori_estimation}
...
\subsection{概率监督学习}
\label{subsec:probabilistic_supervised_learning}
...
\subsection{\gls{SVM}}
\label{subsec:support_vector_machines}
...
核技巧观察到许多机器学习算法都可以写成\gls{example:chap5}间\gls{dot_product}的形式。
...
\label{subsec:other_simple_supervised_learning_algorithms}
...
\label{subsec:principal_components_analysis}
...
\label{subsec:local_constancy_and_smoothness_regularization}
...
\label{subsec:manifold_learning}
