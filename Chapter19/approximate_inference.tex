...
在\gls{DL}中，我们通常有一个可见变量$\Vv$的集合和一个\gls{latent_variable}$\Vh$的集合。
...
\fi
	...
	（左）一个\gls{hidden_unit}相互连接的\gls{srbm}\citep{Osindero+Hinton-2008}。由于存在大量的\gls{latent_variable}的\gls{clique}，\gls{latent_variable}的直接连接使得后验分布难以处理。
...
在本章中，我们将会讲到如何通过近似优化来找$q$的方法来推导出不同形式的的\gls{approximate_inference}。
...
不像大多数我们在本章中介绍的其他算法一样，\glssymbol{EM}并不是一个\gls{approximate_inference}算法，但是是一种能够学到近似后验的算法。
% 626
...
我们通常使用\firstgls{inference}这个术语来指代给定一些其它变量的情况下计算某些变量的概率分布的过程。
...
这种\gls{learned}\gls{approximate_inference}策略已经被应用到了其它模型中。
