...
在\secref{sec:learning_about_dependencies}中，通过了解\firstgls{dependency}，我们简要概括了解决这个难点的两种方法。
...
这也意味着能够理解具有丰富结构的高维数据。%??  我觉得还是应该这样翻译
...
我们用一个\gls{directed_graphical_model}来建模这种关系，就如\figref{fig:relay_race_graph}所示。
% 554 end
...
然后，该模型可以捕获任何对（变量$\RSv_i$和$\RSv_j$间接\gls{dependency}，$\RSv_i$和$\RVh$之间直接\gls{dependency}，$\RVv$和$\RSh_j$直接\gls{dependency})之间的\gls{dependency}关系。%??
% 572  end 
...
\gls{latent_variable}除了发挥本来的作用，即能够高效地描述$p(\RVv)$以外，还有一些优势。%??
