...
因此，所得到的$\log p_{\text{model}}(\Vx)$可能不完全对应于有效的\gls{PD}，但随着$c$估计的改进，它将变得越来越接近有效值\footnote{\glssymbol{NCE}}也适用于具有易于处理的，不需要引入额外参数$c$的\gls{partition_function}的问题。它已经是最令人感兴趣的估计具有复杂\gls{partition_function}模型的方法。
...
具体来说，我们引入了第二个分布，\firstgls{noise_distribution}$p_{\text{noise}}(\Vx)$。
...
这样的下界能够用于构建$p_{\text{joint}}( y = 1 | \Vx)$的下界，但是它只能用于构建$p_{\text{joint}}(y = 0 | \Vx)$（出现在一般的\glssymbol{NCE}对象中）的上界。
