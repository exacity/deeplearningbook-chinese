...
另外，尽管（非线性）\gls{CG}算法传统上作为批方法，\gls{minibatch}版本已经成功用于训练神经网络{cite?}。
针对神经网路的\gls{CG}应用早已被提出，例如放缩的\gls{CG}算法{cite?}。
...
\gls{batch_normalization}{cite?}是优化\gls{DNN}中最激动人心的最新创新之一。
...
在\gls{greedy_supervised_pretraining}的原始版本{cite?}中，每个阶段包括一个仅涉及最终神经网络的子集层的监督学习训练任务。
\gls{greedy_supervised_pretraining}的一个例子如\fig?所示，其中每个附加的\gls{hidden_layer}作为浅监督MLP的一部分\gls{pretraining}，以先前训练的\gls{hidden_layer}输出作为输入。
...
\fi
\caption{一种形式的\gls{greedy_supervised_pretraining}的示意图~{cite?}。
(a)我们从训练一个足够浅的架构开始。
(b)同一个架构的另一描绘。
...
一个和跳过连接相关的想法是添加和网络中间\gls{hidden_layer}相连的输出的额外副本，如GoogLeNet{cite?}和深度监督网络{cite?}。
...
\gls{continuation_method}也和参数中加入\gls{noise}的模拟退火紧密相关{cite?}。
...
\gls{curriculum_learning}被证实为与人类教学方式一致{cite?}：
教师刚开始会展示更容易，更典型的示例，然后帮助学习者在不太显然的情况下提炼决策面。
在人类教学上，基于\gls{curriculum_learning}的技巧比基于样本均匀采样的技巧更有效，也能提高其他学习技巧的效率{cite?}。
