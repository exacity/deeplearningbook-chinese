...
\end{equation}
这个正则化策略用于牛顿法的近似，例如Levenberg-Marquardt算法{cite?}，只要Hessian矩阵的负特征值仍然相对接近零，就会效果很好。
...
{Bengio+al-2009}验证这一策略为连续方法，通过增加简单样本的影响（通过分配它们较大的系数到代价函数，或者更频繁地采样），先前的$J^{(i)}$会容易。
...
在人类教学上，基于课程学习的策略比基于样本均匀采样的策略更有效，也能提高其他学习策略的效率{cite?}。
