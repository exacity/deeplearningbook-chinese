...
包含所有这样的偏导数的矩阵被称为雅可比矩阵。
具体来说，如果我们有一个函数：$\Vf: \SetR^m \rightarrow \SetR^n$，$\Vf$的雅可比矩阵$\MJ \in \SetR^{n \times m}$定义为$J_{i,j} = \frac{\partial}{\partial \Sx_j} f(\Vx)_i$。
...
\end{align}
这意味着$H_{i,j} = H_{j,i}$， 因此海森矩阵在这些点上是对称的。
在深度学习背景下，我们遇到的大多数函数的海森几乎处处都是对称的。
因为海森矩阵是实对称的，我们可以将其分解成一组实特征值和特征向量的正交。
...
我们要最小化的函数能用二次函数很好地近似的情况下，海森的特征值决定了学习速率的量级。
...
利用海森的特征值分解，我们可以将二阶导数测试扩展到多维情况。
在临界点处（$\nabla_{\Vx} f(\Vx) = 0$），我们通过检测海森的特征值来判断该临界点是一个局部极大点、局部极小点还是鞍点。
当海森是正定的（所有特征值都是正的），则该临界点是局部极小点。
因为方向二阶导数在任意方向都是正的，参考单变量的二阶导数测试就能得出此结论。
同样的，当海森是负定的（所有特征值都是负的），这个点就是局部极大点。
在多维情况下，实际上可以找到确定该点是否为鞍点的积极迹象（某些情况下）。
如果海森的特征值中至少一个是正的且至少一个是负的，那么$\Vx$是$f$某个横截面的局部极大点，却是另一个横截面的局部极小点。
见\fig?中的例子。
...
\fi
\caption{梯度下降无法利用包含在海森矩阵中的曲率信息。
这里我们使用梯度下降来最小化海森矩阵条件数为5的二次函数$f(\Vx)$。
...
与指向该方向的特征向量对应的海森的大的正特征值表示该方向上的导数快速增加，因此基于海森的优化算法可以预测，在此情况下最陡峭方向实际上不是有前途的搜索方向。
}
\end{figure}

<!-- % -- 86 -- -->

使用海森矩阵的信息指导搜索可以解决这个问题。
其中最简单的方法是牛顿法。
...
如\eqn?所讨论的，当附近的临界点是最小点（海森的所有特征值都是正的）时牛顿法才适用，而梯度下降不会被吸引到鞍点(除非梯度指向鞍点)。

仅使用梯度信息的优化算法被称为\textbf{一阶优化算法}(first-order optimization algorithms)，如梯度下降。
使用海森矩阵的优化算法被称为\textbf{二阶最优化算法}(second-order optimization algorithms){cite?}，如牛顿法。
