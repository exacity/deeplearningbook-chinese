...
稍加改进的随机梯度下降算法仍然是当今深度学习的主要训练算法。

很多模型基于感知机和ADALINE中所使用的函数$f(\Vx, \Vw)$，统称为线性模型。
...
联结主义者开始研究实际能基于神经实现的认知模型{cite?}，其中很多复苏的想法可以追溯到心理学家Donald Hebb在20世纪40年代的工作{cite?}。

联结主义的中心思想是，当网络将大量简单的计算单元连接在一起时可以实现智能行为。
计算模型中隐藏单元与生物神经系统中的神经元起着类似作用，因此这种见解同样适用。
...
深度学习迅速崛起的激动人心的一幕是卷积网络第一次大幅赢得这一挑战，将最高水准的前5错误率从26.1\%降到15.3\%{cite?}，这意味着该卷积网络针对每个图像的可能类别生成一个顺序列表，除了15.3\%的测试样本，其他测试样本的正确类标都出现在此列表中的前5项里。
...
这种序列到序列的学习似乎引领着另一个应用的演进：机器翻译{cite?}。
