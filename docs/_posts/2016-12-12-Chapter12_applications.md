...
深度学习的基本思想基于连接机制：尽管机器学习模型中单个生物性的神经元或者说是单个特征不是智能的，但是大量的神经元或者特征作用在一起往往能够表现出智能。
...
在这种情况下，使用选通器不会降低计算成本，但如果每个样本的选通器选择单个专家，我们就会获得一个特殊的硬专家混合体, % 刚性~{cite?}，这可以加速推断和训练的时间。
...
不同形式的专用硬件{cite?}的研究已经持续了好几十年，比如专用集成电路的数字（基于数字的二进制表示），模拟{cite?}（基于作为电压或电流连续值的物理实现）和混合实现（组合数字和模拟组件）。
近年来更灵活的可编程门阵列实现（其中电路的具体细节可以建立后写入芯片）也得到了长足发展。
...
训练$n$-gram模型是简单的，因为最大似然估计可以简单地统计每个可能的$n$-gram在训练集中出现的次数。                                                                                                                                                                                                                                                                                                                                                                                       
...
不同于基于类的$n$-gram模型，神经语言模型在识别两个相似的词的基础上，而不丧失将每个词编码为彼此不同的能力。
...
短列表方法的一个明显缺点是，神经语言模型模型的潜在泛化优势仅限于最常用的词，这大概是最没用的。
...
当然即使使用分层softmax，计算所有$|\SetT|$个词的概率仍将是昂贵的。
...
注意机制在计算上比直接索引期望的$\Vh^{(t)}$更昂贵，但直接索引不能使用梯度下降训练。
...
SVD和式 \eq?中的双线性预测在Netflix奖（目的是仅基于大量匿名用户的之前评级预测电影的评级）的竞争中表现得非常好{cite?}。
...
专用深度学习架构，如卷积网络已经应用于从丰富内容中提取特征，如提取用于音乐推荐的音乐音轨{cite?}。
...
事实上，建模语言和通过关系编码建模知识的联系非常接近，研究人员可以\emph{同时}使用知识图谱\emph{和}自然语言句子训练这样的实体表示{cite?}，或组合来自多个关系型数据库的数据~{cite?}。
