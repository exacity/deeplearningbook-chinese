...
RBM的隐藏单元在给定可见单元的情况下彼此条件独立，是一个典型的具有简单正相和困难负相的模型。
...
<!-- % 599 end -->
实现\eqn?的一个朴素方法是，每次需要计算梯度时，预烧随机初始化的一组马尔可夫链。
...
<!-- % -- 599 -- -->
\begin{algorithm}[ht]
\caption{一种朴素的MCMC算法，使用梯度上升最大化具有难以计算配分函数的对数似然。}
\begin{algorithmic}
...
我们可以将MCMC方法视为在两种力之间平衡最大似然，一种力拉高数据出现时的模型分布，一种拉低模型采样出现时的模型分布。
\fig?展示了这个过程。
...
对于大部分深度概率模型来说，马尔可夫链是基于Gibbs采样的，并且对于单个变量产生非局部的移动但是无法同时移动所有的变量。
...
他们认为，由于偏差较小，CD可以作为一种计算代价低的方式来初始化模型，之后可以通过计算代价高的MCMC方法进行微调。
...
CD算法可以被理解为惩罚具有快速改变来自数据的输入的马尔可夫链的模型。
这意味着使用CD训练有点类似于训练自编码器。
即使CD估计比一些其他训练方法具有更大偏差，但是它有助于预训练之后会堆叠起来的浅层模型。
这是因为堆栈中最早的模型会受激励复制更多的信息到其潜变量，使其可用于随后的模型。
这应该更多地被认为是CD训练中经常可利用的副产品，而不是原本的设计优势。
...
此外，因为可以存储所有采样变量的状态，无论是可见的还是潜在的，SML为隐藏单元和可见单元都提供了初始值。
...
在$k$太小或$\epsilon$太大时，随机梯度算法移动模型的速率比马尔可夫链在迭代步中混合更快，此时SML容易变得不准确。
...
NCE能够非常成功地应用于随机变量较少的问题，但即使随机变量很多时，它也很有效。
...
如果$p_{\text{model}}$学会了眼睛，就算没有学习任何其他面部特征，如嘴， 它也会拒绝几乎所有的非结构化噪扰样本。
