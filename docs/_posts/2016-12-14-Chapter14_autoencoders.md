...
传统自编码器被用于降维或特征学习。
近年来，自编码器与潜变量模型理论的联系将自编码器带到了生成建模的前沿，我们将在\chap?看到更多细节。
...
通用近似定理, %万能逼近定理保证至少有一层隐藏层且隐藏单元足够多的前馈神经网络能以任意精度近似任意函数（在很大范围里），这是非平凡深度的一个主要优点。
...
如\sec?描述，自编码器跟其他很多机器学习算法一样，也应用了将数据集中在一个低维流形或者一小组这样的流形的思想。
