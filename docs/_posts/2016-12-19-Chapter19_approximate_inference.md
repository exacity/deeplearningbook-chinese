...
这些相互作用产生于当V-结构的子节点可观察时一个潜变量与另一个潜变量或者更长的激活路径相连。
...
$f$函数的函数被称为是泛函 $J[f]$。
正如我们许多情况下对一个函数求关于以向量为变量的偏导数一样，我们可以使用泛函导数，即关于任意特定的$\Vx$值，对一个泛函 $J[f]$求关于函数$f(\Vx)$的导数，这也被称为变分微分。
...
当我们的图模型包含了连续型潜变量时，我们仍然可以通过最大化$\CalL$进行变分推断和学习。
