...
RNN也可以应用于跨越两个维度的空间数据（如图像），并且当应用于涉及时间的数据时，该网络可具有在时间上向后的连接，前提是将整个序列提供给网络之前能观察到整个序列。
...
例如，如果在统计语言建模中使用的RNN，通常给定前一个词预测下一个词，可能没有必要存储$t$前输入序列中的所有信息；而仅仅存储足够预测句子其余部分的信息。
...
我们使用Teacher Forcing的最初动机是为了在缺乏隐藏到隐藏连接的模型中避免通过时间反向传播。
...
计算图的节点包括参数$\MU,\MV,\MW, \Vb$和$\Vc$，以及以$t$为索引的顺序节点$\Vx^{(t)}, \Vh^{(t)},\Vo^{(t)}$和$L^{(t)}$。
...
等效地，假设给定时刻$t$的变量后，时刻$t +1$变量的条件概率分布是平稳的，这意味着之前的时间步与下个时间步之间的关系并不依赖于$t$。
...
另一种选择是在模型中引入一个额外的Bernoulli输出，表示在每个时间步决定继续或停止。
...
权重与输入保持独立。
我们可以把这种模型视为采用非条件模型的$\Vtheta$并使之成为$\Vomega$，其中$\Vomega$内的偏置参数现在是输入的函数。
...
Jacobian矩阵告诉我们$\Vh^{(t)}$一个微小的变化如何向前传播，或等价的，$\Vh^{(t+1)}$的梯度如何向后传播。
...
还有学者提出{cite?}（相对于隐藏单元）截断反向传播梯度，但没有发布与这些变种之间的比较; 我们推测，所有这些方法表现类似。
...
特别是即使损失函数只对序列未部的输出作惩罚，我们也希望梯度向量$\nabla_{\Vh^{(t)}} L$在反向传播时能维持其幅度，。
形式的，我们要使
\begin{align}
