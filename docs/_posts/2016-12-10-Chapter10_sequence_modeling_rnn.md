...
读者可以参考~{Schmidhuber96,ElHihi+Bengio-nips8}或~{Jaeger2007}了解更早的关于深度~RNN~的研究。

{Graves-arxiv2013}第一个展示了将~RNN~的状态分为多层的显著好处，如\fig?~\emph{(左)}。
...
LSTM~已经在许多应用中取得重大成功，如无约束手写识别{cite?}、语音识别{cite?}、手写识别{cite?}、机器翻译{cite?}、为图像生成标题{cite?}和解析{cite?}。
...
{Greff-et-al-arxiv2015}发现其中的关键因素是遗忘门，而~{Jozefowicz-et-al-ICML2015}发现向LSTM遗忘门加入1的偏置(由~{Gers-et-al-2000}提倡)能让~LSTM~变得与已探索的最佳变种一样健壮。
