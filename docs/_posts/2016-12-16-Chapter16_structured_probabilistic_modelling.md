...
例如， 当我们需要对一个单独的标量变量$\RSx\in\SetR$建模，并且这个包含一个点的团势能定义为$\phi(x) = x^2$时。
在这种情况下，
\begin{align}
Z = \int x^2 dx.
\end{align}
...
值得注意的是，我们可以通过令$\phi$等于对应负能量的指数来获得图\?中的$\phi$函数，比如，$\phi_{\RSa,\RSb}(\RSa,\RSb) = \exp(-E(\RSa,\RSb))$。}
\end{figure}
<!-- %  562 head -->
...
"d"代表"依赖性"的意思。
有向图中d-分离的定义与无向模型中分离的定义相同：我们认为如果图结构显示给定另外的变量集$\SetS$时 $\SetA$与变量集$\SetB$无关，
...
重要的是要记住分离和d-分离只能告诉我们\emph{图中隐含}的条件独立性。
图并不需要表示所有存在的独立性。 
...
但是，一些模型很适合使用有向图\emph{描述}，而另一些模型很适用于使用无向模型描述。
<!-- % 564 end -->
...
而无向模型公式通常对于推导近似推断过程（我们将在\chap?中看到，\eqn?强调了无向模型的作用）是很有用的。
<!-- % 566 -->
...
得到的图$\CalU$被称为是道德图。
关于一个通过道德化将有向图模型转化为无向模型的例子可以参见\fig?。
<!-- % 567 -->
...
（右）通常讲，道德化的过程会给图添加许多边，因此丢失了一些隐含的独立性。
...
（中）为了将无向图转化为有向图，我们必须通过保证所有长度大于$3$的环都有弦来三角形化图。
...
因子图是从无向模型中抽样的另一种方法，它可以解决标准无向模型句法中图表达的模糊性。
...
（左）一个包含三个变量（$\RSa$、$\RSb$和$\RSc$）的团组成的无向网络。
（中）对应这个无向模型的因子图。
这个因子图有一个包含三个变量的因子。
（右）对应这个无向模型的另一种有效的因子图。
...
通常$\RVv$的不同元素彼此高度依赖。
在深度学习中，最常用于建模这些依赖性关系的方法是引入几个潜在的或"隐藏"变量$\RVh$。
然后，该模型可以捕获任何对之间的依赖性关系（变量$\RSv_i$和$\RSv_j$间接依赖，$\RSv_i$和$\RVh$之间直接依赖，$\RVv$和$\RSh_j$直接依赖)。
<!-- % 572  end  -->

一个好的不包含任何潜变量的 关于$\RVv$的模型需要在贝叶斯网络中的每个节点具有大量父节点或在马尔可夫网络中具有非常大的团。
...
潜变量还有一个额外的优势，即能够高效地描述$p(\RVv)$。
新变量$\RVh$还提供了$\RVv$的替代表示。
例如，如\sec?所示，GMM学习了一个潜变量，这个潜变量对应于输入样本是从哪一个混合体中抽出。
这意味着GMM中的潜变量可以用于做分类。
在\chap?中，我们看到了简单的概率模型如稀疏编码是如何学习可以用作分类器输入特征或者作为流形上坐标的潜变量的。
...
然而，在深度学习中，我们通常对如何组合这些工具做出不同的设计，导致总体算法、模型与更传统的图模型具有非常不同的风格。
<!-- % 575 head -->
...
分布式表示有一个缺点就是对于精确推断和环状信念传播等传统技术来说很难使用不够稀疏的图。
...
该模型能够进行高效的Gibbs采样，并且模型设计的重点在于以很高的自由度来学习潜变量，潜变量的含义并不是设计师指定。
...
（左）用MNIST训练模型，然后用Gibbs采样进行采样。
每一列是一个单独的Gibbs采样过程。
每一行表示另一个$1000$步后Gibbs采样的输出。
连续的样本之间彼此高度相关。
（右）对应的权重向量。
将本图结果与图\?中描述的线性因子模型的样本和权重相比。
由于RBM的先验$p(\Vh)$没有限制为因子，这里的样本表现得更好。
