...
\fi
	\caption{训练好的RBM及其权重中的样本。（左）用MNIST训练模型，然后用Gibbs采样进行采样。每一列是一个单独的Gibbs采样过程。没遗憾表示另一个$1000$步Gibbs采样的输出。连续的样本之间彼此高度相关。（右）对应的权重向量。将本图结果与图\?中描述的样本和权重相比。由于RBM的先验$p(\Vh)$没有限制为因子的，这里的样本表现得更好。采样时RBM能够学习到哪些特征需要一起出现。另一方面说，RBM后验$p(\Vh \mid \Vv)$是因子的的，而稀疏编码的后验并不是，所以在特征提取上稀疏编码模型表现得更好。其它的模型可以使用非因子的的$p(\Vh)$和非因子的的$p(\Vh \mid \Vh)$。图片的复制经过{lisa_tutorial_rbm}的允许。}
\end{figure}
