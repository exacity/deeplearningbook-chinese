...
\gls{ML}中的许多重要工具都基于从某种分布中采样以及用这些样本对目标量做一个\gls{monte_carlo}估计。  %??  是 换成 都是，还是觉得别扭，要不改成  都是以。。。为基础的  或者  基于。。。（上）发展的。  你斟酌下。
...
有时候，我们使用它加速一些很费时却易于处理的求和估计，就像我们使用\gls{minibatch}对整个训练代价进行\gls{subsample}一样。  %?? 和的估计  换成  求和估计
在其他情况下，我们需要近似求一个难以处理的和或积分，例如估计一个\gls{undirected_model}中\gls{partition_function}对数的梯度时。   %?? 去掉 时，sum 翻译成 求和，可以和 积分 成对，后面也有类似情况，你斟酌下
...
一种更加通用的方式是构建一个收敛到目标分布估计的序列。 %?? 一种更加通用的方法是构建一个收敛于目标分布的估计序列。
...
$p(\Vx)f(\Vx)$存在不唯一的分解因为它总是可以被写成    %?? 通常  换成  总是
\begin{align}
...
\end{align}
然而，\gls{importance_sampling}的方差却对$q$的选择非常敏感。  %?? 去掉 不同，  选取 换成 选择
这个方差可以表示为
\begin{align}
...
一个更好的\gls{importance_sampling}分布会把更多的权重放在被积函数较大的地方。   %?? 好 换成  更好
事实上，当$f(\Vx)$的正负符号不变时，$\Var[\hat{s}_{q^*}]=0$， 这意味着当使用最优的$q$分布时，\emph{只需要一个样本就足够了}。 %?? 去掉 采
当然，这仅仅是因为计算$q^*$时已经解决了所有的问题。   
所以在实践中这种只需要采样一个样本的方法往往是无法实现的。 %?? 需要采 换成 需要 或是 采样， 另外可以考虑换个语序， 所以在实践中，这种。。。。
% 584
...
所以这种估计也被称为渐进性\gls{unbiased}的。   %?? 可以考虑加个 gls 渐进无偏
% 585
...
例如，\gls{importance_sampling}被应用于加速训练具有大规模词汇的神经网络\gls{language_model}的过程中（见\secref{sec:importance_sampling_chap12}）或者其他有着大量输出结点的\gls{NN}中。   
...
在这种情况下更加频繁地抽取这些困难的样本可以降低梯度估计的方差~\citep{Hinton06}。 %??  在这种情况下，更多地采样难以分类的样本有利于降低梯度方差~\citep{Hinton06}。
% 586 
...
目前为止我们已经了解了如何通过反复更新$\Vx \xleftarrow{} \Vx'\sim T(\Vx'\mid\Vx)$从一个分布$q(\Vx)$中采样。 %?? 截至目前 换成 目前为止
...
当我们考虑\gls{gibbs_sampling}算法（见\secref{sec:gibbs_sampling}）时，这种现象格外明显。 %??  当我们考虑\gls{gibbs_sampling} 。。。。
