...
只要$\text{Var}[f(\Vx^{(i)})]<\infty$，方差$\text{Var}[f(\RVx^{(i)})]$减小并收敛到0：
\begin{align}
...
为了采$\RSa$，我们必须先从$p(\RSa\vert \RSb)$中采样，为了采$\RSb$，我们又必须从$p(\RSb\vert \RSa)$中采样。
...
\begin{align}
q' (\RVx') = \SetE_{\RVx\sim q}T(\RVx'\vert \RVx)
\end{align}
...
\firstgls{gibbs_sampling}是一种概念简单而又有效的方法来构造一个从$p_{\text{model}}(\Vx)$中采样的\gls{markov_chain}，其中在\gls{energy_based_model}中从$T(\RVx'\vert \RVx)$采样是通过选择一个变量$\RSx_i$然后从$p_{\text{model}}$中的该点在无向图$G$中邻接点的条件分布中抽样。
...
给定$\RSb$时的条件分布满足$p(\RSb=1\vert \RSa=1) = \sigma(w)$。
如果$w$的值很大，sigmoid趋近于饱和，那么$b$取到1的概率趋近于1。
相同的道理，如果$\RSa=-1$，那么$\RSb$取到$-1$的概率也趋于1。
根据模型$p_{\text{model}}(\RSa,\RSb)$，两个变量取一样的符号的概率几乎相等。
根据$p_{\text{model}}(\RSa\vert \RSb)$，两个变量应该有相同的符号。
