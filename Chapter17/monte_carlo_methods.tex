...
当$\Vx$是高维数据时，$q$分布的简单性使得它很难于$p$或者$p\vert f\vert$相匹配。
...
所以这个过程收敛到\firstgls{stationary_distribution}，有时也称为\firstgls{equilibrium_distribution}。
收敛时，我们得到
\begin{align}
\label{eqn:1723}
\Vv ' = \MA \Vv = \Vv,
...
在这种情况下，一些研究者把这种\gls{markov_chain}称为\firstgls{harris_chain}，但是这两种情况我们都用\gls{markov_chain}来表示。
...
像这样的同时更新许多变量的\gls{gibbs_sampling}通常被称为\firstgls{block_gibbs_sampling}。
% 590
