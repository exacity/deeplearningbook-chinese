...
\glssymbol{DBM}是\gls{energy_based_model}，意味着模型变量的\gls{joint_probability_distribution}由\gls{energy_function} $E$参数化。
...
这种\gls{greedy_layer_wise_training}过程不仅仅是坐标上升。
...
因此\citet{ranzato+mnih+hinton:2010-short} 也倡导通过哈密尔顿（混合）\gls{monte_carlo}直接采样$p(\Vx)$。%??  此处漏了引用
...
这种方法只适用于非常小的网络。
另一种方法是使用学成的\gls{inference}机制，如\secref{sec:learned_approximate_inference}中描述的。
