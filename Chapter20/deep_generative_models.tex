...
算法|||c|||给出了应用于\glssymbol{DBM}的变分\gls{SML}算法。
...
我们还可以训练高阶\gls{BM}\citep{sejnowski1986higher} ，其中\gls{energy_function}项涉及许多变量的乘积。
...
回想\eqnref{eq:3.47}，对于可求反的、可微的、连续的$g$，
\begin{align}
...
\centerline{\includegraphics{figure.pdf}}
\else
\centerline{\includegraphics{Chapter20/figures/kingma-vae-2d-faces-manifold}}
\fi
\caption{TODO}
...
在实际的实验中，\glssymbol{GAN}博弈的最佳表现形式既不是零和也不等价于最大似然，而是\citet{Goodfellow-et-al-NIPS2013-small}引入的带有启发式动机的不同形式化。
...
\centerline{\includegraphics{figure.pdf}}
\else
\centerline{\includegraphics{Chapter20/figures/lsun_small}}
\fi
\caption{TODO}
