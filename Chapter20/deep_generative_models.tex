...
但是像\glssymbol{RBM}一样，每一层内的每个变量是相互独立的，并条件于相邻层中的变量。%??  在给定相邻层中变量的条件下， 每一层内的每个变量是相互独立的。
...
与\glssymbol{RBM}的\gls{energy_function}(\eqnref{eq:205e})相比，\glssymbol{DBM}\gls{energy_function}包括权重矩阵($\MW^{(2)}$和$\MW^{(3)}$)形式的\gls{hidden_unit}（\gls{latent_variable}）之间的连接。%??  以权重矩阵的形式表示\gls{hidden_unit}（\gls{latent_variable}）之间的连接
...
立即可得，当我们条件于偶数层中的变量时，奇数层中的变量变得条件独立。%??  立即可得 改成 容易发现    变得 改成 是...的   
当然，当我们条件于奇数层中的变量时，偶数层中的变量也会变得条件独立。%?? 同上
...
\glssymbol{RBM}允许所有可见单元以一个块的方式更新，而所有\gls{hidden_unit}在第二个块更新。%??   a second  翻译成  另一个更好？
...
\glssymbol{DBM}一个不幸的特性是从中采样是相对困难的。%??  不幸  换成 不理想
\glssymbol{DBN}只需要在其顶部的一对层中使用\glssymbol{mcmc}采样。
其他层仅在采样过程结束时，只需在一个有效的\gls{ancestral_sampling}步骤中使用\glssymbol{mcmc}。%??  mcmc 无中生有，我觉得改成如下更好： 其他层仅在采样过程末尾涉及，并且只需在一个高效的\gls{ancestral_sampling}过程。
...
\begin{align}
 \CalL(Q, \Vtheta) = \sum_i \sum_{j^{\prime}} v_i \MW_{i,j^{\prime}}^{(1)} 
...
由于\gls{DBM}包含\gls{RBM}作为组件，用于计算\gls{RBM}的\gls{partition_function}和采样的困难同样适用于\gls{DBM}。%??  适用于  改为  困扰着  是否更好？
...
然而实际上，在\gls{greedy_layer_wise_training}过程的情况下，我们在每个步骤使用不同的\gls{objective_function}。
%?? 这句翻译偏差较大，好像是句子换了。  
%?? The two methods differ because the greedy layer-wise training procedure uses a different objective function at each step.  这两种方法是不同的，因为\gls{greedy_layer_wise_training}过程中在每一步都是用了不同的\gls{objective_function}。
