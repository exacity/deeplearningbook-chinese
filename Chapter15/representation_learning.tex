...
即使我们可能没有将语言$X$中的单词$A$翻译成语言$Y$中的单词$B$的\gls{labeled}样本，我们也可以\gls{generalized}并猜出单词$A$的翻译，这是由于我们已经学习了语言$X$和$Y$单词的\gls{distributed_representation}，并且通过两种语言句子的匹配对组成的训练样本，产生了关联于两个空间的链接（可能是双向的）。
...
具体来说，如果表示向量$\Vh$表示观察值$\Vx$的很多根本因素，并且输出向量$\Vy$是最为重要的原因之一，那么从$\Vh$预测$\Vy$会很容易。
% 532 mid 
...
\gls{generative_adversarial_networks}会在\setref{sec:generative_adversarial_networks}中更详细地介绍。
