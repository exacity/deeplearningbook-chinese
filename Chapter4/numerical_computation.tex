...
对数字计算机来说实数无法在有限内存下精确表示，所以计算涉及实数的函数也是困难的。
...
\begin{algorithm}[ht]
\caption{An algorithm to minimize
$ f(\Vx) = \frac{1}{2} || \MA \Vx - \Vb ||_2^2$ with respect to $\Vx$
using gradient descent, starting from an arbitrary value of $\Vx$.
}
\label{alg:gdlsq}
\begin{algorithmic}
\STATE Set the step size ($\epsilon$) and tolerance ($\delta$) to small,
positive numbers.
