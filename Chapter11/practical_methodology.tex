...
\item 根据具体观察反复地逐步改动，如收集新数据、调整\gls{hyperparameter}，或改进算法。
\end{itemize}
...
我们有几种不同的\gls{performance_metrics}，可以用来度量含有\gls{ML}应用的性能。
...
检测器永远报告没有患者，可能会有一个完美的\gls{precision}，但\gls{recall}为零。
...
有可能权衡\gls{coverage}和\gls{precision}。
