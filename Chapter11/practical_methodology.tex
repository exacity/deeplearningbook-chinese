...
\gls{batch_normalization}有时也能降低\gls{generalization_error}，允许省略\gls{dropout}步骤，这是因为用于归一化变量的统计量估计存在\gls{noise}，有助于泛化。 %?? 还是有问题
% -- 413 --  end
...
一个可以替代收集更多数据的简单方法是降低模型大小或是改进\gls{regularization}（调整\gls{hyperparameter}，如\gls{weight_decay}系数，或是加入\gls{regularization}策略，如\gls{dropout}）。%?? 这句结构太乱
...
然而，如果训练算法不能找到某个合适的函数来最小化训练\gls{cost}，或是如果\gls{regularization}项（如\gls{weight_decay}）排除了这些合适的函数，那么即使模型的表达能力较高，也不能学习出合适的函数。%??  还是太乱
...
和\gls{grid_search}不同，我们\emph{不需要离散化}\gls{hyperparameter}的值。这允许我们在一个更大的集合上进行搜索，而不产生额外的计算代价。%?? bin 不知道如何翻译bin，你有什么好的翻译没 
...
因此，优化涉及到探索（探索高度不确定的\gls{hyperparameter}，可能带来显著的效果提升，也可能效果很差）和使用（使用已经确信效果不错的\gls{hyperparameter}——通常是先前见过的非常熟悉的\gls{hyperparameter}）之间的权衡。%??  略乱。    额，不知道如何改进。。。
...
我们可以设计一种足够简单的情况，能够提前得到正确结果，判断模型预测是否与之相符；我们也可以设计一个测试，独立检查神经网络实现的一部分。%??  判断和模型输出是否相符 这多了一句     改成上述，帮助理解
