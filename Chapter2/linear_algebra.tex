...
线性代数提供了被称为\firstgls{matrix_inverse}的强大工具，使我们能够解析地求解\eqnref{eq:2.11}中的多值$\MA$。
...
\centerline{\includegraphics{figure.pdf}}
\else
\centerline{\includegraphics{Chapter2/figures/empty2}}
\fi
\caption{TODO}
...
现在我们可以通过如果以下步骤求解\eqnref{eq:2.11}：
\begin{gather}
\MA\Vx=\Vb \\
\MA^{-1}\MA\Vx = \MA^{-1}\Vb \\
\MI_{\Sn} \Vx=\MA^{-1}\Vb \\
...
这是\eqnref{eq:2.11}对于每一个向量$\Vb$的取值都有解的充足条件。
