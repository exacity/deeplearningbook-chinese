...
卷积就可以很直接地应用；核依据输入的大小简单地被使用不同次，并且卷积运算的输出也相应地缩放。
...
\begin{itemize}
  ...
  将类似的\gls{attention_mechanisms}融入深度学习模型是一个活跃的研究方向。
  在深度学习的背景下，\gls{attention_mechanisms}对于自然语言处理是最成功的，参考\secref{sec:using_an_attention_mechanism_and_aligning_pieces_of_data}。
