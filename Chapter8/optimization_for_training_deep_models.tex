...
反之，机器学习通常会优化一个替代损失函数，并停止在基于提前终止（\secref{sec:early_stopping}）的收敛条件满足的情况。
...
因此，泛化误差的准确梯度的无偏估计可以通过从数据生成分布$p_\text{data}$抽取\gls{minibatch}样本$\{ \Vx^{(1)}, \dots,\Vx^{(m)} \}$以及对应的标签$y^{(i)}$，然后计算该\gls{minibatch}上损失函数对应的参数的梯度
\begin{equation}
\label{eq:8.9}
...
\REQUIRE Initial parameter $\Vtheta$
\WHILE{stopping criterion not met}% NOTE: the while is lower case in the text,
% so the condition should be too
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding targets $\Vy^{(i)}$.
    \STATE Compute gradient estimate: $\hat{\Vg} \leftarrow + 
         \frac{1}{m} \nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$
    \STATE Apply update: $\Vtheta \leftarrow \Vtheta - \epsilon \hat{\Vg}$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\REQUIRE Initial parameter $\Vtheta$, initial velocity $\Vv$.
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding targets $\Vy^{(i)}$.
    \STATE Compute gradient estimate: $\Vg \leftarrow 
         \frac{1}{m} \nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$
    \STATE Compute velocity update: $\Vv \leftarrow \alpha \Vv - 
    \epsilon \Vg$
    \STATE Apply update: $\Vtheta \leftarrow \Vtheta + \Vv$ 
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding labels $\Vy^{(i)}$.
    \STATE Apply interim update: $\tilde{\Vtheta} \leftarrow \Vtheta  + \alpha \Vv$
         \STATE Compute gradient (at interim point): $\Vg \leftarrow 
...
如果该随机游动调整到保持范数，那么前馈网络能够很大程度地避免相同权重矩阵用于每层的梯度消失和膨胀问题，如\secref{sec:long_term_dependencies}所述。
...
\STATE Initialize gradient accumulation variable $\Vr = 0$
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding targets $\Vy^{(i)}$.
    \STATE Compute gradient: $\Vg \leftarrow  
         \frac{1}{m} \nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$ 
    \STATE Accumulate squared gradient: $\Vr \leftarrow \Vr + \Vg \odot \Vg$
    \STATE Compute update: $\Delta \Vtheta \leftarrow -
    \frac{\epsilon}{\delta+ \sqrt{\Vr}} \odot\Vg$.   \ \  (Division and square root applied element-wise)
    \STATE Apply update: $\Vtheta \leftarrow \Vtheta + \Delta \Vtheta$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\REQUIRE Initial parameter $\Vtheta$
\REQUIRE Small constant $\delta$, usually $10^{-6}$, used to stabilize division
by small numbers.
\STATE Initialize accumulation variables $\Vr = 0$
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding targets $\Vy^{(i)}$.
    \STATE Compute gradient: $\Vg \leftarrow  
         \frac{1}{m} \nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$ 
    \STATE Accumulate squared gradient: $\Vr \leftarrow \rho
    \Vr + (1-\rho) \Vg \odot \Vg$
    \STATE Compute parameter update: $\Delta \Vtheta =
    -\frac{\epsilon}{\sqrt{\delta + \Vr}} \odot \Vg$.  \ \  ($\frac{1}{\sqrt{\delta + \Vr}}$ applied element-wise)
    \STATE Apply update: $\Vtheta \leftarrow \Vtheta + \Delta \Vtheta$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[ht]
\caption{RMSProp algorithm with Nesterov momentum}
% 2ED: Greg Brockman was confused why we do not build a \Delta variable
% here when we do in many of the other algorithms, including RMSProp.
% IG would prefer to just eliminate \Delta in all of them.
\label{alg:rms_nesterov}
\begin{algorithmic}
\REQUIRE Global learning rate $\epsilon$, decay rate $\rho$, momentum coefficient $\alpha$.
\REQUIRE Initial parameter $\Vtheta$, initial velocity $\Vv$.
\STATE Initialize accumulation variable $\Vr = 0$
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding targets $\Vy^{(i)}$.
    \STATE Compute interim update: $\tilde{\Vtheta} \leftarrow \Vtheta + \alpha \Vv$
    \STATE Compute gradient: $\Vg \leftarrow  
         \frac{1}{m} \nabla_{\tilde{\Vtheta}} \sum_i L(f(\Vx^{(i)};\tilde{\Vtheta}),\Vy^{(i)})$ 
    \STATE Accumulate gradient: $\Vr \leftarrow \rho
    \Vr + (1-\rho) \Vg \odot \Vg$
    \STATE Compute velocity update: $\Vv \leftarrow \alpha \Vv
    -\frac{\epsilon}{\sqrt{\Vr}} \odot \Vg$. \ \  ($\frac{1}{\sqrt{\Vr}}$ applied element-wise)
    \STATE Apply update: $\Vtheta \leftarrow \Vtheta + \Vv$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\REQUIRE Initial parameters $\Vtheta$
\STATE Initialize 1st and 2nd moment variables $\Vs = 0 $, $\Vr = 0$
\STATE Initialize time step $t=0$ 
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Sample a minibatch of $m$ examples from the training set $\{ \Vx^{(1)},
    \dots, \Vx^{(m)}\}$ with corresponding targets $\Vy^{(i)}$.
    \STATE Compute gradient: $\Vg \leftarrow \frac{1}{m} \nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$ 
    \STATE $t \leftarrow t + 1$
    \STATE Update biased first moment estimate: $\Vs \leftarrow \rho_1 \Vs + (1-\rho_1) \Vg$
    \STATE Update biased second moment estimate: $\Vr \leftarrow \rho_2 \Vr + (1-\rho_2) \Vg \odot \Vg$
    \STATE Correct bias in first moment: $\hat{\Vs} \leftarrow \frac{\Vs}{1-\rho_1^t}$
    \STATE Correct bias in second moment: $\hat{\Vr} \leftarrow \frac{\Vr}{1-\rho_2^t}$
    \STATE Compute update: $\Delta \Vtheta = - \epsilon \frac{\hat{\Vs}}{\sqrt{\hat{\Vr}} + \delta}$ \ \  (operations applied element-wise)
    \STATE Apply update: $\Vtheta \leftarrow \Vtheta + \Delta \Vtheta$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[ht]
\caption{Newton's method with objective $J(\Vtheta)= \frac{1}{m} \sum_{i=1}^m L(f(\Vx^{(i)};\Vtheta), y^{(i)})$.}
\label{alg:newton}
\begin{algorithmic}
\REQUIRE Initial parameter $\Vtheta_{0}$
\REQUIRE Training set of $m$ examples
\WHILE{stopping criterion not met}  % NOTE: do not capitalize the condition
    % NOTE: the algorithmic package puts the word "do" in the .pdf file
    % after the contents of the FOR command. So we do not want to put
    % comment marks inside the FOR block. If we do that, it looks like
    % the "do" is commented.
      \STATE Compute gradient: $\Vg \leftarrow 
     \frac{1}{m} \nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$ 
      \STATE Compute Hessian: $\MH \leftarrow  
     \frac{1}{m} \nabla_{\Vtheta}^2 \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$ 
    \STATE Compute Hessian inverse: $\MH^{-1}$
    \STATE Compute update: $\Delta \Vtheta = - \MH^{-1} \Vg$
    \STATE Apply update: $\Vtheta = \Vtheta+\Delta \Vtheta$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
% 2ED: ideally this would use superscript indexing
\label{alg:cg}
\begin{algorithmic}
\REQUIRE Initial parameters $\Vtheta_{0}$
\REQUIRE Training set of $m$ examples
\STATE Initialize $\Vrho_{0} = 0$
\STATE Initialize $g_0 = 0$
\STATE Initialize $t = 1$
\WHILE{stopping criterion not met} % NOTE: do not capitalize the condition
    \STATE Initialize the gradient $\Vg_{t} = 0$
    \STATE Compute gradient: $\Vg_{t} \leftarrow
         \frac{1}{m}\nabla_{\Vtheta} \sum_i L(f(\Vx^{(i)};\Vtheta),\Vy^{(i)})$ 
    \STATE Compute $\beta_{t} = \frac{(\Vg_{t}-\Vg_{t-1})^\top \Vg_{t}}{\Vg_{t-1}^\top \Vg_{t-1}}$  (Polak-Ribi\`{e}re)
    \STATE (Nonlinear conjugate gradient: optionally reset $\beta_{t}$ to zero,
           for example if $t$ is a multiple of some constant $k$, such
               as $k=5$)
    \STATE Compute search direction: $\Vrho_{t} = -\Vg_{t} + \beta_{t} \Vrho_{t-1}$ 
    \STATE Perform line search to find: $\epsilon^{*} = \arg\!\min_{\epsilon}
    \frac{1}{m} \sum_{i=1}^{m}L(f(\Vx^{(i)};\Vtheta_t + \epsilon \Vrho_t),\Vy^{(i)})$ 
    \STATE (On a truly quadratic cost function, analytically solve for $\epsilon^*$
            rather than explicitly searching for it)
    \STATE Apply update: $\Vtheta_{t+1} = \Vtheta_{t}+ \epsilon^{*} \Vrho_{t}$
    \STATE $t \leftarrow t + 1$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
\gls{greedy_supervised_pretraining}的一个例子如\figref{fig:chap8_deep_sup}所示，其中每个附加的\gls{hidden_layer}作为浅监督MLP的一部分提前训练，以先前训练的\gls{hidden_layer}输出作为输入。
