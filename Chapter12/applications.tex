...
尽管\gls{DL}的一个目标是设计能够处理各种任务的算法，然而截止目前\gls{DL}的应用仍然需要一定程度的特殊化。
...
通过调整\gls{fixed_point_arithmetic}的实现方式，\citet{Vanhoucke-et-al-2011}获得了 3倍于一个强\gls{float_point_arithmetic}系统的速度。
...
\glssymbol{GPU} 最初是为图形应用而开发的专用硬件组件。
...
对于\gls{SGD}的单步来说，我们可以增加\gls{minibatch}的大小，但是从优化性能的角度来说，通常我们得到的回报并不会线性增长。
...
\gls{model_compression}的基本思想是用一个更小的模型取代替原始耗时的模型，从而使得用来存储于评估所需的内存与运行时间更少。



当原始模型由于为了防止\gls{overfitting}而变得很大时，\gls{model_compression}可以起到作用。
...
只要我们拟合了这个函数$f(\Vx)$，我们就可以通过将$f$应用于随机采样点$x$来生成有无穷多训练样本的训练集。
%我们就可以生成一个拥有了无穷多训练样本的训练集，只需将$f$作用于任意生成的$\Vx$。
然后，我们使用这些样本训练一个新的更小的模型，使其能够在这些点上拟合$f(\Vx)$。
为了更加充分地利用了这个新的小模型的\gls{capacity}，最好从类似于之后将提供给模型的真实测试数据的分布中采样$\Vx$。
...
\gls{cascade}的另一个版本使用早期模型来实现一种硬\gls{attention_mechanism}：\gls{cascade}的早期成员定位对象，并且\gls{cascade}的后续成员在给定对象位置的情况下执行进一步处理。
...
\citet{bengio-arxiv13-condcomp}提出使用\gls{gater}概率梯度的若干估计器，而\citet{Bacon-et-al-RLDM2015,BengioE-et-al-arXiv2015}使用\gls{RL}技术（\firstgls{policy_gradient}）来学习一种\gls{hidden_unit}块的条件\gls{dropout}的形式，减少了实际的计算成本，而不会对近似的质量产生负面影响。
% 438
...
在一些情况下，可以通过将样本分成组，并且都采用相同的分支并且同时处理这些样本组的方式来缓解这些问题。
...
尽管图像合成通常不包括在\gls{CV}内，但是能够进行图像合成的模型通常用于图像恢复，即修复图像中的缺陷或从图像中移除对象这样的\gls{CV}任务。
% 441 
...
其他种类的预处理被应用于训练集和测试集，其目的是将每个样本置于更规范的形式，以便减少模型需要考虑的变化量。
...
从大图像中剪切感兴趣的对象所组成的数据集不可能包含任何具有几乎恒定强度的图像。
...
\gls{LCN}通常可以通过使用可分离卷积（参见\secref{sec:efficient_convolution_algorithms}）来计算\gls{feature_map}所需要的局部平均值和局部\gls{standard_deviation}，然后使用元素级的减法和除法实现不同的\gls{feature_map}。
% 444
...
比如说，\citet{Robinson+Fallside91}在TIMIT数据集\citep{garofolo1993darpa}（有$39$个区分的\gls{phoneme}\footnote{39}）上达到了$26$\%的\gls{phoneme}错误率，这个结果优于或者说是可以与基于\glssymbol{HMM}的结果相比。
...
在这之前的长达十年左右的时间内，尽管数据集的规模是随时间增长的（见\citet{Deng+Yu-2014}的图2.4），基于\glssymbol{GMM}-\glssymbol{HMM}的系统的传统技术已经停滞不前了。
